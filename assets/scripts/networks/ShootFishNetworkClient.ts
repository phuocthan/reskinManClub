...
import FacebookTracking from "../common/FacebookTracking";
...
export default class ShootFishNetworkClient {
    ...
    private login() {
        ...
        let params = {
            ...
            "password": Configs.Login.Password ? md5(Configs.Login.Password) : "",
            "platform": Configs.App.getPlatformName(),
            "appid": NativeBridge.getPackageName(),
            "s": Configs.Login.IsLoginFB ? "fb" : "",
            "at": Configs.Login.IsLoginFB ? SPUtils.getFbToken() : "" 
        }

        console.log("xxenglogin" + JSON.stringify(params));

        this.request("xxenglogin", params, (res) => {
    ...
    private onClose(ev: Event) { 
        ...
        if(Configs.Login.IsLogin) {
            ...
            //     }
            // });
            FacebookTracking.logDisconnect();
        }
    }
        ...
        if (this.ws == null) {
            ...
            if (this.isUseWSS) {
                if (cc.sys.isNative && cc.sys.os == cc.sys.OS_ANDROID) {
                    this.ws = new (Function.prototype.bind.apply(WebSocket, [null, "ws://" + this.host + ":" + this.port]));
                } else {
    ...
    public request(route: string, data: any, callback: (res: Object) => void, target: cc.Component) {
        ...
        console.log("request:");
        console.log(JSON.stringify({ data: typeof data != "object" || data == null || !data ? {} : data, msgId: ShootFishNetworkClient.reqId, route: route }));
