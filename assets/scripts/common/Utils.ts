...
export namespace common {
    export class Utils {
        ...
        static formatLongNumber(n: number): string {
            ...
            if(n < 1000000) {
                var nint = Math.floor(n/1000);
                return Utils.formatNumber(nint) + "K";
            }
            if(n >= 1000000) {
                var nint = Math.floor(n/1000000);
                return Utils.formatNumber(nint) + "M";
            }
        ...
        static v2LengthSqr(v: cc.Vec2): number {
            return v.x * v.x + v.y * v.y;
        }

        static v2Length(v: cc.Vec2): number {
            return Math.sqrt(this.v2LengthSqr(v));
        }
        ...
        static dateToddMMYYYYHI(date: Date) {
            var mm = date.getMonth() + 1; // getMonth() is zero-based
            var dd = date.getDate();
            var h = date.getHours();
