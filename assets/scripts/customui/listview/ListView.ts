...
export default class ListView extends cc.Component {
    ...
    scrollTopNotifyed: boolean = false;
    scrollBottomNotifyed: boolean = false;
    ...
    init(data?, componentName?) {
        ...
        this.adapter.setDataSet(data, componentName);
    ...
    notifyUpdate(updateIndex?, keepPosition?) {
        if (this.adapter == null) {
            return;
        }
    ...
    updateView(startIndex) {
        ...
        for (let index of updates) {
            ...
            // let child = cc.instantiate(this.itemTemplate);
            this.horizontal ?
