...
namespace taixiumini {
    ...
    export class PopupSoiCau extends Dialog {
        ...
        // hna add
        @property(cc.Label)
        lblLastSessionResult: cc.Label = null;
        // end
        ...
        private drawPage1() {
            ...
            for (var i = data.length - 1; i >= 0; i--) {
                ...
                // hna add
                iconXX123.getChildByName('score').getComponent(cc.Label).string = score.toString();
                // end
        ...
        private drawPage2() {
            ...
            for (let i = 0; i < data.length; i++) {
                for (let j = 0; j < data[i].length; j++) {
                    let score = data[i][j];
                    // let icon = cc.instantiate(this.iconNumberTemplate); // hna comment
                    // hna add
                    let icon = cc.instantiate(score >= 11 ? this.iconTaiTemplate : this.iconXiuTemplate);
                    // end
                    icon.parent = this.contentDraw;
                    icon.position = cc.v2(startPosX + spacingX * i, startPosY - spacingY * j);
                    // icon.color = cc.Color.BLACK.fromHEX(score >= 11 ? "#4192ff" : "#FFFFFF"); // hna comment
                    // icon.getComponent(cc.Label).string = "" + score; // hna comment
                    icon.getChildByName('score').getComponent(cc.Label).string = score.toString();
                }
            }
