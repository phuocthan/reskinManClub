...
export default class Player extends cc.Component {
    ...
    prepareToTransform() {
        for (let index = 0; index < this.cardReal.childrenCount; index++) {
    ...
    getRealCardCanUse() {
        for (let index = 0; index < this.cardReal.childrenCount; index++) {
            if (!this.cardReal.children[index].active) {
                return index;
            }
        }
        return 0;
    ...
    hideFxSpecial() {
        for (let index = 0; index < this.fxSpecial.childrenCount; index++) {
            this.fxSpecial.children[index].active = false;
        }
    }

    checkNeedShowDanBai() {
        if (this.fxSpecial.children[2].active || this.fxSpecial.children[3].active) {  // Xi dzach or Xi bang
            return false;
        }
        return true;
    }
    ...
    shadowCardReady(state) {
        for (let index = 0; index < this.cardReady.childrenCount; index++) {
    ...
    resetCardReal() {
        ...
        this.cardReal.y = 0;
        for (let index = 0; index < this.cardReal.childrenCount; index++) {
            this.cardReal.children[index].children[0].getComponent(cc.Sprite).spriteFrame = this.spriteCardBack;
        }
