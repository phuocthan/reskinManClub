...
export default class Play extends cc.Component {
                ...
                ShootFishNetworkClient.getInstance().ping(() => {
                    ...
                    ShootFishNetworkClient.getInstance().request("play", {
                        ...
                        if (!res["ok"]) {
                            ...
                            switch (res["err"]) {
                                case 4:
                                    App.instance.confirmDialog.show2("Yêu cầu tối thiểu 20.000 Xu. Xin vui lòng nạp thêm.", (isConfirm) => {
                                        if (isConfirm) {
    ...
    private updateShoot(dt: number) {
        ...
        if (this.curShootInterval > 0) {
            ...
            FacebookTracking.betBanCaSuccess(this.listBet[this.betIdx]);
            console.log("betBanCaSuccess: " + this.listBet[this.betIdx]);
        }
    }
