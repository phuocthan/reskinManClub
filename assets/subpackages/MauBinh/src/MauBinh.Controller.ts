...
import Player from "./MauBinh.Player";
...
export default class MauBinhController extends cc.Component {
    ...
    @property(cc.SpriteFrame)
    spriteGroupCardBg: cc.SpriteFrame[] = [];
    ...
    @property(cc.SpriteFrame)
    spriteResultBg: cc.SpriteFrame[] = [];

    @property(cc.SpriteFrame)
    spriteResultText: cc.SpriteFrame[] = [];

    @property(cc.Node)
    doneStamp: cc.Node = null;
    ...
    setTimeWaittingCountDown() {
        ...
        this.notifyTimeStart.getComponentInChildren(cc.Label).string = "Sẵn Sàng: " + this.seconds + "s ";
    }
    ...
    setTimeEndCountDown() {
        ...
        this.notifyTimeEnd.getComponentInChildren(cc.Label).string = "Kết thúc sau : " + this.seconds + "s ";
    }
                        ...
                        for (let a = 0; a < cardDeal; a++) { // players x 4 cards
                            for (let b = 0; b < numPlayer; b++) {
                                ...
                                if (seatId !== -1) {
                                    ...
                                    let rawPlayerPos = this.players[seatId].position.add(this.players[seatId].getComponent(Player).nodeCard.position);
                            ...
                            for (let index = 0; index < numPlayer; index++) {
                                ...
                                if (seatId !== -1) {
                                    ...
                                    if (seatId == 0) {
                                        ...
                                        this.btnSoChi.active = true;
                                        let type = this.getBinhType(mauBinh);
                                        this.getPlayerHouse(0).resetResultGame();
                                        if (mauBinh != Cmd.Code.TYPE_BINH_THUONG) {
                                            this.getPlayerHouse(0).playFxResultGeneral(0, type.bg, type.text, 0);
                                        }
                                        for (let index = 0; index < numPlayer; index++) {
                ...
                case Cmd.Code.TU_DONG_BAT_DAU:
                    {
                        ...
                        FacebookTracking.logCountMauBinh();
                    }
                    break;
                case Cmd.Code.THONG_TIN_BAN_CHOI:
                    {
                        ...
                        console.log("MauBinh_Reconnect: " + JSON.stringify(hasInfo));
                        console.log("MauBinh_Reconnect: " + JSON.stringify(players));
                        ...
                        for (let index = 0; index < configPlayer.length; index++) {
                            ...
                            if (findPos > -1) {
                                ...
                                this.setupSeatPlayer(seatId, {
                                    nickName: players[arrPlayerPosExist[findPos]].nickName,
                                    avatar: parseInt(players[arrPlayerPosExist[findPos]].avatar),
                                    money: players[arrPlayerPosExist[findPos]].currentMoney
                                });

                                if (players[arrPlayerPosExist[findPos]].status == Cmd.Code.PLAYER_STATUS_VIEWER) {
                                    ...
                                    if (seatId != 0) {
                                        this.getPlayerHouse(seatId).showCardReady(false);
                                        this.getPlayerHouse(seatId).showCardReal(true);
                                        if (players[arrPlayerPosExist[findPos]].sochi) {
                                            this.getPlayerHouse(seatId).playFxXepXong();
                                        } else {
                                            this.getPlayerHouse(seatId).playFxDangXep();
                                        }
                                    } else {
                                        this.btnSoChi.active = !players[arrPlayerPosExist[findPos]].sochi;
                                        this.btnCombining.active = players[arrPlayerPosExist[findPos]].sochi;
                                    }
                                }
                            } else {
                                // Not Exist player  -> Active Btn Add player
                        ...
                        if (this.currentCard.length > 0) {
                            ...
                            for (let a = 0; a < this.currentCard.length; a++) {
                                ...
                                if(this.btnCombining.active) {
                                    cardOpen.getComponent("MauBinh.MeCard").offDrag();
                                }
                                this.getPlayerHouse(0).transformToCardReal(a, this.spriteCards[spriteCardId], 0);
                            }
                            this.doneStamp.active = this.btnCombining.active;
                        }
                ...
                case Cmd.Code.DANG_KY_THOAT_PHONG:
                    {
                        ...
                        if (seatId !== -1) {
                            ...
                            if (isOutRoom) {
                                this.getPlayerHouse(seatId).showNotify("Đăng ký rời bàn !");
                            } else {
    ...
    completeMoveCard(targetName) {
        ...
        */
        ...
        let type = this.getBinhType(result.cardType);
        this.getPlayerHouse(0).resetResultGame();
    ...
    getPlayerHouse(seatId) {
        return this.players[seatId].getComponent("MauBinh.Player");
    }
    ...
    getBinhType(maubinhType) {
        let bg = null;
        let text = null;
        switch (maubinhType) {
            case Cmd.Code.TYPE_SANH_RONG:
                bg = this.spriteResultBg[0];
                text = this.spriteResultText[0];
                break;
            case Cmd.Code.TYPE_MUOI_BA_CAY_DONG_MAU:
                bg = this.spriteResultBg[0];
                text = this.spriteResultText[1];
                break;
            case Cmd.Code.TYPE_MUOI_HAI_CAY_DONG_MAU:
                bg = this.spriteResultBg[0];
                text = this.spriteResultText[2];
                break;
            case Cmd.Code.TYPE_BA_CAI_THUNG:
                bg = this.spriteResultBg[0];
                text = this.spriteResultText[3];
                break;
            case Cmd.Code.TYPE_BA_CAI_SANH:
                bg = this.spriteResultBg[0];
                text = this.spriteResultText[4];
                break;
            case Cmd.Code.TYPE_LUC_PHE_BON:
                bg = this.spriteResultBg[0];
                text = this.spriteResultText[5];
                break;
            case Cmd.Code.TYPE_BINH_LUNG:
                bg = this.spriteResultBg[1];
                text = this.spriteResultText[6];
                break;
    ...
    soChi(chiId, playerResultList) {
        ...
        for (let index = 0; index < playerResultList.length; index++) {
            ...
            if (seatId != -1) {
                ...
                if (result.maubinhType == Cmd.Code.TYPE_BINH_THUONG && isNeedSoChi) {
                    ...
                    this.getPlayerHouse(seatId).playFxCompareChi(chiId, this.spriteGroupCardBg[spriteId], this.spriteGroupCard[spriteId]);
                    ...
                    if (chiId == 1) {
                        ...
                        if (result.maubinhType == Cmd.Code.TYPE_BINH_THUONG) {
                            ...
                            if (playerCardInfo.chiDau < 2) {
                                this.getPlayerHouse(seatId).playFxCompareChi(chiId, this.spriteGroupCardBg[playerCardInfo.chiDau], this.spriteGroupCard[playerCardInfo.chiDau]);
                            }

                            if (playerCardInfo.chiGiua < 2) {
                                this.getPlayerHouse(seatId).playFxCompareChi(chiId, this.spriteGroupCardBg[playerCardInfo.chiGiua], this.spriteGroupCard[playerCardInfo.chiGiua]);
                            }

                            if (playerCardInfo.chiGiua == Cmd.Code.GROUP_SAM_CO) {
                                this.getPlayerHouse(seatId).playFxCompareChi(chiId, this.spriteGroupCardBg[playerCardInfo.chiCuoi], this.spriteGroupCard[playerCardInfo.chiCuoi]);
                            }
                        } else {
                            let type = this.getBinhType(result.maubinhType);
                            this.getPlayerHouse(seatId).resetResultGame();
                            this.getPlayerHouse(seatId).playFxResultGeneral(seatId, type.bg, type.text, 1);
                        }
        ...
        if (isNeedSoChi) {
            ...
            // show Gold
            this.showGoldResult(playerResultList, 1);
            this.batSap(playerResultList);
        }
    }
            ...
            for (let index = 0; index < playerResultList.length; index++) {
                ...
                if (seatId != -1) {
                    ...
                    if (result.moneyCommon >= 0) {
                        ...
                        if(seatId == 0) {
                            FacebookTracking.betBinhSuccess(Math.abs(result.moneyCommon));
                        }
                    }
                }
            }
            ...
            for (let index = 0; index < playerResultList.length; index++) {
                ...
                if (seatId == 0) {
                    if (countWin > 0) {
                        ...
                        if (countWin == 3) {
                            // bi_sap_lang
                            this.fxSoChiTotal.getComponent(cc.Sprite).spriteFrame = this.spriteResultBg[0];
                            this.fxSoChiTotal.children[0].getComponent(cc.Sprite).spriteFrame = this.spriteSoChiTotal[0];
                        } else {
                            // sap_3_chi
                            this.fxSoChiTotal.getComponent(cc.Sprite).spriteFrame = this.spriteResultBg[1];
                            this.fxSoChiTotal.children[0].getComponent(cc.Sprite).spriteFrame = this.spriteSoChiTotal[2];
                        }
                    ...
                    if (countLose > 0) {
                        ...
                        if (countLose == 3) {
                            // bat_sap_lang
                            this.fxSoChiTotal.getComponent(cc.Sprite).spriteFrame = this.spriteResultBg[1];
                            this.fxSoChiTotal.children[0].getComponent(cc.Sprite).spriteFrame = this.spriteSoChiTotal[1];
                            this.fxSoChiTotal.runAction(
                                cc.sequence(
                                    cc.scaleTo(0.25, 1.1, 1.1),
                                    cc.scaleTo(0.25, 1, 1),
                    ...
                    if (playerResultList[index].moneySap < 0) {
                        // sap_3_chi
                        this.getPlayerHouse(seatId).playFxSoChiTotal(this.spriteResultBg[1], this.spriteSoChiTotal[2]);
                    }
                }
            }
        } else {
    ...
    getNetWorkClient() {
        return MauBinhNetworkClient.getInstance();
    }
    ...
    showToast(message: string, delay: number = 2) {
        ...
        this.scheduleOnce(() => {
            if(this.toast) {
                // hna add
                this.toast.runAction(
                    cc.moveTo(0.5, 0, Configs.App.DEVICE_RESOLUTION.height/2 - this.toast.height/2 + 120)
                );
                // end
