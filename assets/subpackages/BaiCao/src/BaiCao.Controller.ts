...
import Player from "./BaiCao.Player";
...
export default class BaiCaoController extends cc.Component {
    ...
    setupMatch(data: cmd.ReceivedJoinRoomSucceed) {
        ...
        this.labelRoomId.string = "BÀI CÀO - P: " + roomId;
    ...
    processBetting(rate) {
        ...
        // cc.log("BaiCao processBetting fillRange : ", this.actionBetting.children[1].getComponent(cc.Sprite).fillRange);
    ...
    showPopupInvite(inviteData: CardGameCmd.ReceivedGetInfoInvite) {
        ...
        PopupInvite.createAndShow(this.popup, () => {
            ...
            PopupInvite.instance.setListener((listNickNames) => {
                BaiCaoNetworkClient.getInstance().send(new CardGameCmd.SendInvite(listNickNames));
            });
        });
    }
    ...
    showPopupAcceptInvite(acceptData: CardGameCmd.ReceivedInvite) {
        PopupAcceptInvite.createAndShow(this.popup, () => {
            ...
            PopupAcceptInvite.instance.setListener(() => {
                BaiCaoNetworkClient.getInstance().send(new CardGameCmd.SendAcceptInvite(acceptData.inviter));
            });
        }, "Bài Cào");
    }

    onInviteClick() {
        BaiCaoNetworkClient.getInstance().send(new CardGameCmd.SendGetInfoInvite());
    }
}
