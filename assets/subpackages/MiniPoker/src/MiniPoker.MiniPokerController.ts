...
export default class MiniPokerController extends MiniGame {
    ...
    private readonly rollStartItemCount = 20;
    ...
    private readonly defaultCards = [0, 10, 15, 20, 25];
    ...
    start() {
        ...
        for (let i = 0; i < this.columns.childrenCount; i++) {
            ...
            for (let j = 0; j < count; j++) {
                ...
                if (j >= 1) {
                    ...
                    item.children[0].getComponent(cc.Sprite).spriteFrame = this.sprAtlasCards.getSpriteFrame("card_" + this.defaultCards[i]);
                }
            }
        }
        ...
        this.toggleAuto.node.on("click", () => {
            ...
            if (this.toggleAuto.isChecked) {
                ...
                FacebookTracking.spinMinipokerAuto();
            } else {
        ...
        this.toggleBoost.node.on("click", () => {
            AudioManager.getInstance().playEffect(this.soundClickBtn);
            if (this.toggleBoost.isChecked) {
                if (this.isSpined) this.spin();
                this.toggleAuto.interactable = false;

                FacebookTracking.spinMinipokereFast();
            } else {
    ...
    actSpin() {
        AudioManager.getInstance().playEffect(this.soundClickBtn);
        this.spin();
        FacebookTracking.spinMinipokerClick();
    }
    ...
    private spin() {
        ...
        // this.btnSpinAnim.play("spin", 0); // hna comment
        this.quayAnim.addAnimation(0, "animation", false); // hna add
        this.isSpined = false;
        ...
        FacebookTracking.spinMinipoker();
    }
    ...
    private setEnableAllButtons(isEnable: boolean) {
        ...
        // this.spineSpin.node.active = isEnable; // hna comment
    ...
    private onSpinResult(data: cmd.ReceiveSpin) {
        ...
        FacebookTracking.spinMinipokerSuccess(this.listBet[this.betIdx]);
        FacebookTracking.spinMinipokerBet(this.listBet[this.betIdx]);
        ...
        for (let i = 0; i < this.columns.childrenCount; i++) {
            ...
            roll.runAction(cc.sequence(
                ...
                cc.callFunc(() => {
                    ...
                    bottomSprite.spriteFrame = topSprite.spriteFrame = this.sprAtlasCards.getSpriteFrame("card_" + result[i]);
                })
            ));
        }
    }
    ...
    private spined() {
        ...
        if (this.lastSpinRes.prize > 0) {
            ...
            this.lblWinCash.getChildByName("prize").getComponent(cc.Label).string = "+" + this.lastSpinRes.prize;
            this.lblWinCash.position = cc.v2(0, 42);
            this.lblWinCash.runAction(cc.sequence(
                cc.delayTime(0.5),
                cc.moveBy(1, cc.v2(0, 110)),
                cc.delayTime(1.5),
                cc.callFunc(() => {
    ...
    showPopupRank() {
        AudioManager.getInstance().playEffect(this.soundClickBtn);
        cc.loader.loadRes("prefabs/PopupMiniPoker/PopupHonors", cc.Prefab, (c, t, item) => { }, (err, prefab) => {
            let popup = cc.instantiate(prefab).getComponent("MiniPoker.PopupHonors");
    ...
    showPopupGuide() {
        AudioManager.getInstance().playEffect(this.soundClickBtn);
        cc.loader.loadRes("prefabs/PopupMiniPoker/PopupGuide", cc.Prefab, (c, t, item) => { }, (err, prefab) => {
    ...
    showPopupHistory() {
        AudioManager.getInstance().playEffect(this.soundClickBtn);
        cc.loader.loadRes("prefabs/PopupMiniPoker/PopupHistory", cc.Prefab, (c, t, item) => { }, (err, prefab) => {
            let popup = cc.instantiate(prefab).getComponent("MiniPoker.PopupHistory");
            this.hubPopupsGame.addChild(popup.node);
            popup.show();
        });
    }
